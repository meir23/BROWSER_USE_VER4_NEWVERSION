...
from google.genai import types  # Type definitions for the Gemini API
...
def generate(user_image_path=None, user_text="INSERT_INPUT_HERE"):
    ...
    full_response = ""  # Variable to collect the complete response
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        ...
        generate(user_image_path=sys.argv[1], user_text=sys.argv[2])
    elif len(sys.argv) > 1:
        # If only image path is provided, use default text
        generate(user_image_path=sys.argv[1])
    else:
